...
println "큰 따옴표 사용"
println '작은 따옴표 사용가능!'
println "큰따옴표 안에 '작은 따옴표'를 넣을 수 있다."
...
//3) 문자열 사이에 변수 값을 넣기 => GString(GroovyString)
name ="홍길동"
...
println "오늘 날짜(파라미터 없는 클로저) : ${new Date()} 입니다."	// 리턴 값: 오늘 날짜
println "오늘 날짜(파라미터 없는 클로저) : ${new Date(); 'haha'} 입니다."	// 리턴 값: haha

// 클로저에 파라미터가 있으면 그 파라미터 값이 출력된다.
// 단, << 연산자를 사용하여 파라미터에 리턴 값을 넣어야 한다.
println "오늘 날짜(파라미터 클로저) : ${ok -> '하하하'} 입니다."
println "오늘 날짜(파라미터 클로저) : ${ok -> ok << '헐...'} 입니다."

// 파라미터에 값을 넣은 후 다른 문장을 실행할 때
// => 다른 문장은 실행된다. 단 리턴값은
println "오늘 날짜(파라미터 클로저) : ${ok -> ok << '넣었다'; println 'hahaha'; 'okok'} 입니다."

// GString의 클로저에서 파라미터를 리턴 용으로 사용할 때는
// << 연산자를 이용하여 값을 할당해야 한다.

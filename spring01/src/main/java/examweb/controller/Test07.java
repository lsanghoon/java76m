...
public class Test07 {
	 ...
	 */
	@RequestMapping("/e1")
	public String e1(String name, int age) {
	 ...
	 * => @RequestParam 의 defaultValue 속성 사용
	 ...
	 * 	  			만약 이 속성을 지정하지 않으면, 
	 *          메서드 파라미터 이름과 같은 요청 파라미터(getParameter("xxx"))를 찾는다.
	 *    required : 요청 파라미터 값의 필수 여부 지정
	 *    					 @RequestParam 애노테이션을 붙이면, 기본으로 필수 항복 이 된다.
	...
	@RequestMapping("/e4")
	public String e4(
			@RequestParam(value="userNam", required=false) String name,
			@RequestParam(name="userAge", required=false, defaultValue = "0") int age) {

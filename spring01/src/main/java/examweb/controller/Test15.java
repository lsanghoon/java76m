...
public class Test15 {
  ...
  public String s2(String name, Model model) throws Exception {
  	// 기본으로 프론트 컨트롤러는 Model 객체에 저장된 값을
  	// HttpServletRequest 보관소로 옮긴다.
  	// => 단, @SessionAttributes에 지정된 속성인 경우 , HttpSession으로 옮긴다.
  	model.addAttribute("name", name);
  	return "test15/s2";
  }
  
  @RequestMapping(value="s3")
  public String s3(int age, Model model) throws Exception {
  	model.addAttribute("age", age);
  	return "test15/s3";
  }
  
  @RequestMapping(value="s4")
  public String s4(String tel, Model model, HttpSession session) throws Exception {
  ...
  public String s5(HttpSession session, SessionStatus status) throws Exception {
  	// 세션 무효화시킴
  	// => HttpSession객체에 직접 저장한 값만 무효화 된다.
  	// => Model 객체와 @SessionAttributes 통해 세션에 보관된 값은 무효화 

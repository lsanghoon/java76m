...
public class Exam01 {
	...
	public static void main(String[] args) {
		...
		// - java에서는 primitive type(원시타입)을 제외한 모든 메모리는 new 사용하여 준비한다.
		
		//int a[] = 10;	//배열을 저장하는 변수에는 반드시 배열을 줘야 한다. 값을 주면 오류!
		//long[] sub = new int[7];	//배열 종류에 맞춰서 배열 변수에 담아야 한다. 다르면 오류!
		
		//int sub[] = new int[7];	// 아래와 동일한것이다.
		...
		sub[6] = sub[5] / 5;
		//sub[7] = 100;	// 컴파일할 때 오류는 발생하지 않지만, 실행하면 인덱스 오류가 발생
		for (int i = 0; i < 5; i++) {
			System.out.println(sub[i]);
		}
		System.out.printf("총점 : %d, 평균 : %d", sub[5], sub[6]);

/*
 * 멀티 태스킹?
 * - CPU(Core)를 가지고 여러 프로그램을 동시에 실행하는 방법.
 * - 동시에 실행하는 것처럼 보이는 기술.
 ...
 * 			우선순위가 높은 프로그램을 먼저 실행하는 방법.
 * 			물론, 일정 시간이 지나면 다음 순위의 프로그램을 실행한다.
 * 			단, 우선 순위가 높은 프로그램을 더 자주 실행한다.
 * 			단점, 우선순위가 낮으면 계속 실행이 연기되는 문제가 있다.
 ...
 * 2) context - 프로세스 실행정보
 * 3) context switching) - centext 정보를 로딩 및 저장
 * 		- 다른 포르세스로 CPU 사용권을 넘기기 위해 실행 정보(컨텍스트)를 교체하는 작업
 * 
 * 스레드
 * - OS로 부터 독립적으로 CPU 사용권을 받아서 실행하는 코드이다.
 * - 멀티 태스킹 구현하는 핵심 기술.
 * 
 * 자바에서 스레드를 구현하는 방법
 * 1) Thread를 상속받기

...
public class Exam10 {
	static int v1;	// 클래스 변수 : 한번 생성되면 프로그램을 종료할 때 까지 유지된다.
	...
	public static void main(String[] args) {
		...
		int v3 = 30;	// 로컬 변수는 메서드를 실행하는 동안만 유지된다.
									// 메서드 호출이 끝나면 사라진다.
		{
			int v4 = 40;	// 블록안에 선언된 로컬 변수는 블록이 끝나는 순간 제거된다.
			{
				int v5 =50;	//
			...
			System.out.println(v3);
			System.out.println(v4);
			//System.out.println(v5);	// 오류! 블록을 벗어나면 해당 로컬변수를 사용할수 없다.
			System.out.println("-----------");
		}
		System.out.println(v3);
		//System.out.println(v4);	// 오류! 블록을 벗어나면 해당 로컬변수를 사용할수 없다.
		System.out.println("-----------");
	}

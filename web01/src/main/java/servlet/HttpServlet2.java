...
public abstract class HttpServlet2 extends GenericServlet {
	...
	// 기능: 클라이언트가 요청한 방식을 구분하여 해당 메서드를 구분한다.
	public void service(
...
/*	private void printError(
		...
		out.printf("%s는 지원하지 않습니다.", request.getMethod());
	}*/

	// HttpServlet2를 상속 받는 서브 클래스에서 다음 메서드 중 하나를 재정의 할 것이다.
	// GET 요청을 처리하고 싶다면 doGet()을 재정의 할 것이고,
	// POST 요청을 처리하고 싶다면 doPost()를 재정의 할 것이다.
	// 왜? 위의 service() 메서드에서 요청 방식에 따라 아래 메서드를 호출하도록 프로그램을 짰기때문이다.
	//
	...
	public void doHead(
			HttpServletRequest request, HttpServletResponse response)
					throws IOException, ServletException {
		throw new ServletException("이 서블릿은 HEAD을 지원하지 않습니다.");
	}
